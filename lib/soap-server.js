...
SoapServer.prototype.handleOperationRequest = function(service, soapRequest){
	...
	for(var operationRequestName in operationRequest){
		console.log(`operation: ${operationRequestName} requested`);
		console.log(JSON.stringify(operationRequest));
		...
		for(var inputName in operation.inputs){			
			...
			var requestArgument;
			// check for the nil == true element
			if (requestArguments[inputNamePrefix+inputName][0] && requestArguments[inputNamePrefix+inputName][0]["$"] && requestArguments[inputNamePrefix+inputName][0]["$"]["a:nil"] && (requestArguments[inputNamePrefix+inputName][0]["$"]["a:nil"] == "true"))
			{				
				requestArgument = null;				
			}
			else
			{
				requestArgument = this.castArgument(inputName, requestArguments[inputNamePrefix+inputName], input);	
			}
		...
		*/
		...
		return new Promise(function (resolve, reject) {
			var operationResponsePromise = operation.exec.apply(operation, operationArguments);
			operationResponsePromise.then(function (operationResponse) {
				var response = {};
				var responseName = operationName + 'Response';
				response[responseName] = {[operationName + 'Result']: operationResponse};
				resolve(response);
...
SoapServer.prototype.js2xml = function(js, namespace, indentLevel, addTargetNamespace){
	var ret = '';
	var indent = '';
	...
	if('string' === typeof js){
		...
		for(var node in js){
			...
			if(js[node].constructor === Array) {
				ret += indent + '<' + element
				if (addTargetNamespace) {
					ret += ' xmlns="http://'+this.options.tns+'"';
				}
				ret += '>\n';
				for(var i = 0; i < js[node].length; i++) {
					ret += this.js2xml(js[node][i], namespace, indentLevel + 1, false);
				}
				ret += indent + '</' + element + '>\n';
			}
			else if('object' == typeof js[node]) {
				ret += indent + '<' + element
				if (addTargetNamespace) {
					ret += ' xmlns="http://'+this.options.tns+'"';
				}
				ret += '>\n';
				ret += this.js2xml(js[node], namespace, indentLevel + 1, false);
				ret += indent + '</' + element + '>\n';
			}
			else {
				ret += indent + '<' + element + '>' + '<![CDATA[' + js[node] +  ']]>' + '</' + element + '>\n';
			}
...
SoapServer.prototype.handleRequest = function(req, res){
	try{
		this.handleSoapRequest(req, function(response, httpCode){
			if (!httpCode)
				httpCode = 200;
			res.writeHead(httpCode, {'Content-Type': 'text/xml'});
			res.end(response);
		});
	}
	catch(exception){
...
SoapServer.prototype.handleSoapRequest = function(req, callback){
	...
	if (req.method == 'POST') {
		...
		req.on('end', function () {
			...
			parseXml(body, {ignoreAttrs: true}, function (err, soapRequest) {
				if(err){
					//console.dir('Request parse error: ' + err);
					throw err;
				}

				var responseXml = '<s:Envelope\n';
				responseXml += '	xmlns:s="http://schemas.xmlsoap.org/soap/envelope/"\n';
				responseXml += '	>\n';
				responseXml += '	<s:Body>\n';
				...
				// 	responseXml += This.js2xml(soapResponse, null, 2, true);
				// }
				// catch(exception){
				...
				// 	httpCode = 500;
				// 	responseXml += '	<s:Fault>\n';
				// 	responseXml += '		<faultcode>' + exception.faultCode + '</faultcode>\n';
				// 	responseXml += '		<faultstring>' + exception.faultString + '</faultstring>\n';
				// 	responseXml += '		<detail>\n';
				...
				// callback.apply(null, [responseXml, httpCode]);

				var soapResponsePromise = This.handleOperationRequest(service, soapRequest);
				soapResponsePromise.then(function(soapResponse) {
					responseXml += This.js2xml(soapResponse, null, 2, true);
					...
					if (exception.name != 'SoapServiceException') {
						httpCode = 500;
						responseXml += '	<s:Fault>\n';
						responseXml += '		<faultstring>' + exception.message + '</faultstring>\n';
						responseXml += '	</s:Fault>\n';
						responseXml += '	</s:Body>\n';
						responseXml += '</s:Envelope>';
					}
...
SoapServer.prototype.getWsdl = function(host, service){
	...
	for(var operationName in service.operations){
		var operation = service.operations[operationName];
		var operationObjects = service.operations[operationName].getObjects();
		for(var objectName in operationObjects){
			//objects[objectName] = operationObjects[objectName];
			var object = operationObjects[objectName];
			wsdl += '			<xsd:element name="' + objectName + '" type="tns:' + objectName + '">\n';
			wsdl += '			<xsd:complexType name="' + objectName + '">\n';
			wsdl += '				<xsd:sequence>\n';			
		...
		if (operation.hasNoArguments)
		{
			wsdl += '			<xsd:element name="' + operationName + '">\n';
			wsdl += '			<xsd:complexType>\n';
			wsdl += '				<xsd:sequence />\n';												
			wsdl += '			</xsd:complexType>\n';
			wsdl += '			</xsd:element>\n';
	...
	for(operationName in service.operations){
		...
		if (operation.hasNoArguments)
		{
			wsdl += '		<part name="' + operationName + '" type="tns:' + operationName + '" />\n';
		}
		wsdl += '	</message>\n';
